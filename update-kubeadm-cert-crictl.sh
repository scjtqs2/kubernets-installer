...
cert::update_etcd_cert() {
  ...
  crictl --runtime-endpoint unix:///var/run/containerd/containerd.sock ps | awk '/etcd-/{print$(NF-1)}' |xargs -r -I '{}' crictl --runtime-endpoint unix:///var/run/containerd/containerd.sock stopp {} >/dev/null 2>&1 || true
...
cert::update_master_cert() {
  ...
  for item in "apiserver" "controller-manager" "scheduler"; do
    ...
    crictl --runtime-endpoint unix:///var/run/containerd/containerd.sock ps | awk '/kube-'${item}'-/{print $(NF-1)}' | xargs -r -I '{}' crictl --runtime-endpoint unix:///var/run/containerd/containerd.sock stopp {} >/dev/null 2>&1 || true
